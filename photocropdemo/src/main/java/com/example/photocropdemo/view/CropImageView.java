package com.example.photocropdemo.view;
...
import com.example.photocropdemo.R;
...
public class CropImageView extends ImageView {
    ...
    private int mBorderWidth = 3;         //焦点边框的宽度（画笔宽度）
    ...
    private boolean isFocusRectZoom = false;//是否是焦点框的缩放
    private static Handler mHandler = new InnerHandler();
    private final float offset = 40f;
    ...
    private boolean isRectBorder(MotionEvent event ,RectF rect) {
    	if (isRectBorderT(event, rect)) {
    		return true;
		}else if(isRectBorderB(event, rect)) {
			return true;
		}else if(isRectBorderL(event, rect)) {
			return true;
		}else if(isRectBorderR(event, rect)) {
			return true;
		}
    	return false;
    }
    private boolean isRectBorderT(MotionEvent event ,RectF rect) {
    	if ( Math.abs(event.getY() - rect.top) < offset 
    			&& Math.abs(event.getX() - offset) < rect.right
    			&& Math.abs(event.getX() - offset) > rect.left) {
    		return true;
		}
    	return false;
    }
    private boolean isRectBorderB(MotionEvent event ,RectF rect) {
    	if ( Math.abs(event.getY() - rect.bottom) < offset
    			&& Math.abs(event.getX() - offset) < rect.right
    			&& Math.abs(event.getX() - offset) > rect.left) {
    		return true;
		}
    ...
    private boolean isRectBorderL(MotionEvent event ,RectF rect) {
    	if ( Math.abs(event.getX() - rect.left) < offset
    			&& Math.abs(event.getY() - offset) < rect.bottom
    			&& Math.abs(event.getY() - offset) > rect.top) {
    		return true;
		}
    	return false;
    }
    private boolean isRectBorderR(MotionEvent event ,RectF rect) {
    	if ( Math.abs(event.getX() - rect.right) < offset
    			&& Math.abs(event.getY() - offset) < rect.bottom
    			&& Math.abs(event.getY() - offset) > rect.top) {
    		return true;
		}
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
            	if (isFocusRectZoom) {
            		if (isRectBorderT(event, mFocusRect)) {
            			if (mFocusRect.bottom - event.getY() >= minWidth
            					&& event.getY() > 2
            					&& event.getY() >= getImageMatrixRect().top) {
            				mFocusRect.top = event.getY();
                			invalidate();
						}
					}else if (isRectBorderB(event, mFocusRect)) {
						if (event.getY() - mFocusRect.top >= minWidth
								&& event.getY() <= getImageMatrixRect().bottom) {
							mFocusRect.bottom = event.getY();
	            			invalidate();
						}
					}else if (isRectBorderL(event, mFocusRect)) {
						if (mFocusRect.right - event.getX() >= minWidth
								&& event.getX() >= getImageMatrixRect().left) {
							mFocusRect.left = event.getX();
	            			invalidate();
						}
            			
					}else if (isRectBorderR(event, mFocusRect)) {
						if (event.getX() - mFocusRect.left >= minWidth
								&& event.getX() <= getImageMatrixRect().right) {
							mFocusRect.right = event.getX();
	            			invalidate();
						}
					}
					break;
                ...
                if (mode == DRAG) {
//                	if (!isImgRectContain(mFocusRect, getImageMatrixRect())) {
//						break;
//					}
                	Log.i("swm", "-------------x="+(event.getX() - pA.x)+"--------:"+pA.x);
                	Log.i("swm", "-------------y="+(event.getY() - pA.y)+"--------:"+pA.y);
...
//        ViewCompat.postInvalidateOnAnimation(this);
     ...
     * 修正焦点框的缩放比例
     */
    private void fixFocusRectScale() {
    	if (mFocusRect.top < getImageMatrixRect().top) {
    		mFocusRect.top = getImageMatrixRect().top;
		}
    	if (mFocusRect.left < getImageMatrixRect().left) {
    		mFocusRect.left = getImageMatrixRect().left;
    	}
    	if (mFocusRect.bottom > getImageMatrixRect().bottom) {
    		mFocusRect.bottom = getImageMatrixRect().bottom;
    	}
    	if (mFocusRect.right > getImageMatrixRect().right) {
    		mFocusRect.right = getImageMatrixRect().right;
    	}
    	invalidate();
